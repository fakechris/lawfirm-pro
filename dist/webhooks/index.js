...
const webhookHandler = new WebhookHandler_1.WebhookHandler();
const gateway = new gateway_1.IntegrationGatewayService();
const logger = new logger_1.IntegrationLoggerImplementation();
router.post('/webhook', async (req, res) => {
    ...
    try {
        ...
        const payload = {
            id: req.headers['x-webhook-id'] || requestId,
            type: req.headers['x-webhook-type'] || 'unknown',
            timestamp: new Date(),
            data: req.body,
...
router.post('/webhook/stripe', async (req, res) => {
    ...
    try {
        ...
        const payload = {
            id: req.headers['stripe-id'] || requestId,
            type: `stripe.${req.headers['stripe-type'] || 'unknown'}`,
            timestamp: new Date(),
            data: req.body,
...
router.post('/webhook/paypal', async (req, res) => {
    ...
    try {
        ...
        const payload = {
            id: req.headers['paypal-id'] || requestId,
            type: `paypal.${req.headers['paypal-event-type'] || 'unknown'}`,
            timestamp: new Date(),
            data: req.body,
            signature: req.headers['paypal-signature'],
            headers: {
                'paypal-signature': req.headers['paypal-signature'],
                'content-type': req.get('Content-Type'),
                'user-agent': req.get('User-Agent')
            }
        };
...
router.post('/webhook/court', async (req, res) => {
    ...
    try {
        ...
        const payload = {
            id: req.headers['court-id'] || requestId,
            type: `court.${req.headers['court-event-type'] || 'unknown'}`,
            timestamp: new Date(),
            data: req.body,
            signature: req.headers['court-signature'],
            headers: {
                'court-signature': req.headers['court-signature'],
                'content-type': req.get('Content-Type'),
                'user-agent': req.get('User-Agent')
            }
        };
...
router.get('/webhooks', async (req, res) => {
    try {
        ...
        const webhooks = await webhookHandler.listWebhooks(service);
        res.json({
            success: true,
            data: webhooks
        });
...
router.delete('/webhooks/:webhookId', async (req, res) => {
    try {
        ...
        const success = await webhookHandler.unregisterWebhook(service, webhookId);
        if (success) {
            res.json({
                success: true,
...
router.post('/webhooks/:webhookId/test', async (req, res) => {
    try {
        ...
        const success = await webhookHandler.testWebhook(service, webhookId);
        if (success) {
            res.json({
                success: true,

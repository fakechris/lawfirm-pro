...
const documents_1 = require("../../src/services/documents");
const document_processing_1 = require("../../src/utils/document-processing");
const documents_2 = require("../../src/services/documents");
const documents_3 = require("../../src/services/documents");
const documents_4 = require("../../src/services/documents");
...
describe('Document Management System Integration Tests', () => {
    ...
    beforeAll(async () => {
        ...
        documentService = new documents_1.DocumentService(prisma);
        ocrService = new document_processing_1.OCRService();
        searchService = new documents_2.DocumentSearchService(prisma);
        performanceService = new documents_3.DocumentPerformanceService(prisma);
        securityService = new documents_4.DocumentSecurityService(prisma);
    });
    afterAll(async () => {
    ...
    describe('Document Upload and OCR Processing', () => {
        it('should upload document and process OCR successfully', async () => {
            ...
            const result = await documentService.uploadDocument(testBuffer, 'test-legal-document.txt', 'text/plain', {
        ...
        it('should handle OCR processing for different languages', async () => {
            ...
            const result = await documentService.uploadDocument(testBuffer, 'test-chinese-document.txt', 'text/plain', {
                uploadedBy: 'test-user-id',
        ...
        it('should reprocess OCR for existing document', async () => {
            ...
            const testBuffer = Buffer.from(testContent);
            const uploadResult = await documentService.uploadDocument(testBuffer, 'ocr-reprocess-test.txt', 'text/plain', {
                uploadedBy: 'test-user-id',
    ...
    describe('Document Search and Indexing', () => {
        it('should index document for search', async () => {
            ...
            const testBuffer = Buffer.from(testContent);
            await documentService.uploadDocument(testBuffer, 'searchable-document.txt', 'text/plain', {
                uploadedBy: 'test-user-id',
        ...
        it('should search by OCR text', async () => {
            ...
            await documentService.uploadDocument(testBuffer, 'ocr-search-test.txt', 'text/plain', {
                uploadedBy: 'test-user-id',
    ...
    describe('Document Performance Optimization', () => {
        it('should measure document operation performance', async () => {
            ...
            const testBuffer = Buffer.from(testContent);
            const { result, metrics } = await performanceService.measurePerformance('document_upload', () => documentService.uploadDocument(testBuffer, 'performance-test.txt', 'text/plain', {
                uploadedBy: 'test-user-id',
        ...
        it('should process documents in batches', async () => {
            ...
            const results = await performanceService.processBatch(testDocuments, async (doc) => {
                const buffer = Buffer.from(doc.content);
                const result = await documentService.uploadDocument(buffer, doc.name, 'text/plain', {
                    uploadedBy: 'test-user-id',
    ...
    describe('Document Collaboration Features', () => {
        it('should handle document sharing', async () => {
            ...
            const testBuffer = Buffer.from(testContent);
            const uploadResult = await documentService.uploadDocument(testBuffer, 'sharing-test.txt', 'text/plain', {
                uploadedBy: 'test-user-id',
        ...
        it('should handle document comments', async () => {
            ...
            const testBuffer = Buffer.from(testContent);
            const uploadResult = await documentService.uploadDocument(testBuffer, 'comments-test.txt', 'text/plain', {
                uploadedBy: 'test-user-id',
    ...
    describe('Document Versioning', () => {
        it('should create document versions', async () => {
            ...
            const uploadResult = await documentService.uploadDocument(originalBuffer, 'version-test.txt', 'text/plain', {
                uploadedBy: 'test-user-id',
            ...
            const versionResult = await documentService.createVersion(document.id, newBuffer, 'version-test-v2.txt', 'text/plain', 'Updated content and formatting');
        ...
        it('should download specific document versions', async () => {
            ...
            const originalBuffer = Buffer.from(originalContent);
            const uploadResult = await documentService.uploadDocument(originalBuffer, 'version-download-test.txt', 'text/plain', {
                uploadedBy: 'test-user-id',
            ...
            const newBuffer = Buffer.from(newContent);
            await documentService.createVersion(document.id, newBuffer, 'version-download-test-v2.txt', 'text/plain', 'Second version for download test');
    ...
    describe('Error Handling and Edge Cases', () => {
        it('should handle invalid file types', async () => {
            ...
            const result = await documentService.uploadDocument(maliciousBuffer, 'malicious.exe', 'application/x-executable', {
                uploadedBy: 'test-user-id',
        ...
        it('should handle files that are too large', async () => {
            ...
            const result = await documentService.uploadDocument(largeBuffer, 'large-file.txt', 'text/plain', {
                uploadedBy: 'test-user-id',
        ...
        it('should handle invalid tags', async () => {
            ...
            const testBuffer = Buffer.from(testContent);
            const result = await documentService.uploadDocument(testBuffer, 'invalid-tags-test.txt', 'text/plain', {
                uploadedBy: 'test-user-id',
        ...
        it('should handle duplicate files', async () => {
            ...
            const testBuffer = Buffer.from(testContent);
            const firstResult = await documentService.uploadDocument(testBuffer, 'duplicate-test.txt', 'text/plain', {
                uploadedBy: 'test-user-id',
                description: 'First upload'
            });
            expect(firstResult.success).toBe(true);
            const secondResult = await documentService.uploadDocument(testBuffer, 'duplicate-test-v2.txt', 'text/plain', {
                uploadedBy: 'test-user-id',
...
//# sourceMappingURL=documentManagement.test.js.map
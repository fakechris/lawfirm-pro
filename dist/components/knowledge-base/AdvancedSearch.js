...
const AdvancedSearch = ({ onSearch, onClose, }) => {
    ...
    return (<div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-lg shadow-xl max-w-4xl w-full max-h-[90vh] overflow-y-auto">
        ...
        <div className="p-6 space-y-6">
          ...
          <div>
            ...
            <div className="relative">
              <input type="text" value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} placeholder="输入搜索关键词..." className="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500" onKeyPress={(e) => e.key === 'Enter' && handleSearch()}/>
              <div className="absolute right-3 top-1/2 transform -translate-y-1/2 flex space-x-2">
                {searchQuery && (<button onClick={() => setSearchQuery('')} className="text-gray-400 hover:text-gray-600">
        ...
        ].map((type) => (<label key={type.value} className={`flex items-center p-3 border rounded-lg cursor-pointer transition-colors ${filters.contentTypes?.includes(type.value)
                ? 'border-blue-500 bg-blue-50'
                : 'border-gray-300 hover:border-gray-400'}`}>
                  <input type="checkbox" checked={filters.contentTypes?.includes(type.value) || false} onChange={(e) => {
          ...
          {!isLoadingMetadata && categories.length > 0 && (<div>
              <h3 className="text-lg font-medium text-gray-900 mb-4">分类</h3>
              <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
                {categories.map((category) => (<label key={category.id} className={`flex items-center p-3 border rounded-lg cursor-pointer transition-colors ${filters.categories?.includes(category.id)
                    ? 'border-green-500 bg-green-50'
                    : 'border-gray-300 hover:border-gray-400'}`}>
                    <input type="checkbox" checked={filters.categories?.includes(category.id) || false} onChange={(e) => {
          ...
          <div>
            ...
            { value: 'CONFIDENTIAL', label: '机密', color: 'bg-red-100 text-red-800' },
                      ...
                      <input type="checkbox" checked={filters.accessLevels?.includes(level.value) || false} onChange={(e) => {
            ...
            { value: 'year', label: '今年' },
                      ...
                      <input type="radio" name="dateRange" checked={filters.dateRange === range.value} onChange={() => handleFilterChange('dateRange', range.value)} className="border-gray-300 text-blue-600 focus:ring-blue-500"/>
          ...
          {searchHistory.length > 0 && (<div>
              ...
              <div className="flex flex-wrap gap-2">
                {searchHistory.map((query, index) => (<button key={index} onClick={() => setSearchQuery(query)} className="px-3 py-1 bg-gray-100 text-gray-700 rounded-full text-sm hover:bg-gray-200 transition-colors">
                    {query}
                  </button>))}
              </div>
            </div>)}
        ...
        <div className="flex items-center justify-between p-6 border-t border-gray-200">
          <div className="flex items-center space-x-4">
            {hasActiveFilters && (<button onClick={clearAllFilters} className="text-sm text-gray-600 hover:text-gray-800">
                清除筛选
              </button>)}
          ...
          <div className="flex items-center space-x-3">
            ...
            <button onClick={handleSearch} disabled={!searchQuery.trim()} className="px-6 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition-colors disabled:opacity-50 disabled:cursor-not-allowed">
              搜索
            </button>
          </div>
        </div>

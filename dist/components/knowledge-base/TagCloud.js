...
const TagCloud = ({ onArticleSelect, }) => {
    const [selectedTag, setSelectedTag] = (0, react_1.useState)(null);
    const [viewMode, setViewMode] = (0, react_1.useState)('cloud');
    const [sortBy, setSortBy] = (0, react_1.useState)('popular');
    const { tags, articles, isLoading, getArticlesByTag, } = (0, useKnowledgeBase_1.useKnowledgeBase)();
    ...
    const sortedTags = react_1.default.useMemo(() => {
    ...
    if (selectedTag) {
        ...
        <div className="p-6 border-b border-gray-200">
          <div className="flex items-center justify-between mb-4">
            <div className="flex items-center space-x-4">
              <button onClick={handleBackToCloud} className="flex items-center text-blue-600 hover:text-blue-700 transition-colors">
              ...
              <div>
                ...
                {selectedTag.description && (<p className="text-sm text-gray-600 mt-1">{selectedTag.description}</p>)}
              </div>
            </div>
          </div>
        ...
        <div className="p-6">
          {isLoading ? (<div className="text-center py-12">
            ...
            </div>) : tagArticles.length === 0 ? (<div className="text-center py-12">
              ...
              {tagArticles.map((article) => (<div key={article.id} className="cursor-pointer hover:opacity-90 transition-opacity" onClick={() => onArticleSelect?.(article)}>
    ...
    return (<div className="bg-white rounded-lg shadow-sm border border-gray-200">
        ...
        <div className="flex items-center justify-between mb-4">
          ...
          <div className="flex items-center space-x-4">
            ...
            <div className="flex items-center bg-gray-100 rounded-lg p-1">
              <button onClick={() => setViewMode('cloud')} className={`p-2 rounded-md transition-colors ${viewMode === 'cloud'
      ...
      <div className="p-6">
        {isLoading ? (<div className="text-center py-12">
            <div className="animate-spin rounded-full h-12 w-12 border-b-2 border-blue-600 mx-auto mb-4"></div>
            <p className="text-gray-600">加载中...</p>
          </div>) : sortedTags.length === 0 ? (<div className="text-center py-12">
            <svg className="w-16 h-16 mx-auto mb-4 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M7 7h.01M7 3h5c.512 0 1.024.195 1.414.586l7 7a2 2 0 010 2.828l-7 7a2 2 0 01-2.828 0l-7-7A1.994 1.994 0 013 12V7a4 4 0 014-4z"/>
            </svg>
            <h3 className="text-lg font-medium text-gray-900 mb-2">暂无标签</h3>
            <p className="text-gray-600">知识库中暂无标签</p>
          </div>) : viewMode === 'cloud' ? (<div className="flex flex-wrap gap-3 justify-center">
            {sortedTags.map((tag) => {
                const fontSize = getTagSize(tag.articleCount, maxCount);
                const colorClass = getTagColor(tag.articleCount, maxCount);
                return (<button key={tag.id} onClick={() => handleTagSelect(tag)} className={`inline-flex items-center px-4 py-2 rounded-full border transition-all hover:scale-105 hover:shadow-md ${colorClass}`} style={{ fontSize: `${fontSize}rem` }}>
                  <span className="font-medium">{tag.name}</span>
                  <span className="ml-2 text-sm opacity-75">({tag.articleCount})</span>
                </button>);
            })}
          </div>) : (<div className="space-y-3">
            {sortedTags.map((tag) => (<div key={tag.id} onClick={() => handleTagSelect(tag)} className="flex items-center justify-between p-4 bg-gray-50 rounded-lg hover:bg-gray-100 transition-colors cursor-pointer border border-gray-200 hover:border-gray-300">
                <div className="flex items-center space-x-4">

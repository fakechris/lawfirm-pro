...
export class TemplateManagementService {
  ...
  async queryTemplates(query: TemplateQuery = {}): Promise<ContentTemplate[]> {
    const where: any = {};
  ...
  // Template Usage
  async useTemplate(templateId: string, contentId: string, variables: Record<string, any>, createdBy: string): Promise<TemplateUsage> {
  ...
  async getTemplateUsages(templateId: string): Promise<TemplateUsage[]> {
  ...
  // Variable Validation
  private validateVariables(variables: Record<string, any>, schema: TemplateVariable[]): { isValid: boolean; errors: string[] } {
    const errors: string[] = [];
  ...
  private validateCustomRules(value: any, validation: any): string | null {
  ...
  async importTemplate(templateData: string, createdBy: string): Promise<ContentTemplate> {
    try {
  ...
  async createTemplateVersion(templateId: string, changeLog: string): Promise<void> {
    ...
    await this.prisma.templateVersion.create({
      data: {
        templateId,
        version: template.usageCount + 1, // Use usage count as version number
        name: template.name,
        description: template.description,
        templateContent: template.templateContent,
        variableSchema: template.variableSchema,

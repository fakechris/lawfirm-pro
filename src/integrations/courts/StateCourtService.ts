...
export class StateCourtService extends BaseExternalService implements CourtIntegration {
    ...
    try {
      ...
      const filingData = {
        ...
        attorneys: document.attorneys.map(a => ({
          ...
          stateLicense: a.contact.email?.split('@')[1] // Simplified state license check
        })),
        metadata: {
  ...
  // State court-specific methods
  async getCaseInformation(state: string, caseNumber: string): Promise<CaseStatus> {
    try {
      ...
      const response = await this.makeRequest<any>(`/${state}/cases/${caseNumber}`);
  ...
  async searchCases(state: string, query: {
    caseNumber?: string;
    partyName?: string;
    attorneyName?: string;
    caseType?: string;
    ...
    try {
      ...
      if (query.caseType) params.append('caseType', query.caseType);
      ...
      const response = await this.makeRequest<any[]>(`/${state}/cases/search?${params.toString()}`);
  ...
  async getAttorneyInformation(state: string, barNumber: string): Promise<Attorney> {
    try {
      if (!this.supportedStates.includes(state)) {
        throw new Error(`State ${state} is not supported`);
      }
      ...
      const response = await this.makeRequest<any>(`/${state}/attorneys/${barNumber}`);
  ...
  async getFilingFees(state: string, courtType: string, caseType: string): Promise<number> {
    try {
      if (!this.supportedStates.includes(state)) {
        throw new Error(`State ${state} is not supported`);
      }
      ...
      const response = await this.makeRequest<any>(`/${state}/fees/${courtType}/${caseType}`);
